...
class ScrollTerror extends StatelessWidget {
  const ScrollTerror({Key? key}) : super(key: key);

  @override
  Widget build(BuildContext context) {
    TerrorData terrorData = TerrorData();
    ...
    return SizedBox(
      ...
      child: ListView.builder(
        ...
        itemCount: terrorData.listaTerror.length,
        itemBuilder: (BuildContext context, index) {
          Map<dynamic, dynamic> imageData = terrorData.listaTerror[index];
          return ImagenTerror(imageData: imageData);
        },
      ),
    );
  }
...
class ImagenTerror extends StatefulWidget {
  ...
  const ImagenTerror({Key? key, required this.imageData}) : super(key: key);
...
class ImagenTopState extends State<ImagenTerror> with TickerProviderStateMixin {
    ...
    return GestureDetector(
      ...
      onTapUp: (_) {
        ...
        Navigator.push(
          context,
          MaterialPageRoute(
            builder: (context) => InfoTerror(imageData: widget.imageData),
          ),
        );
      },
      onTapCancel: () {
...
class InfoTerror extends StatelessWidget {
  final Map<dynamic, dynamic> imageData;

  const InfoTerror({Key? key, required this.imageData}) : super(key: key);
