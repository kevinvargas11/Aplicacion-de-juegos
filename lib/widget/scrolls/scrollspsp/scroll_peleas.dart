...
class ScrollPelea extends StatelessWidget {
  const ScrollPelea({Key? key}) : super(key: key);

  @override
  Widget build(BuildContext context) {
    PeleaData actionData = PeleaData();
    ...
    return SizedBox(
      ...
      child: ListView.builder(
        ...
        itemCount: actionData.listaPelea.length,
        itemBuilder: (BuildContext context, index) {
          Map<dynamic, dynamic> imageData = actionData.listaPelea[index];
          return ImagenPelea(imageData: imageData);
        },
      ),
    );
  }
...
class ImagenPelea extends StatefulWidget {
  ...
  const ImagenPelea({Key? key, required this.imageData}) : super(key: key);

  // Método que crea el estado asociado a este widget.
  @override
  ImagenPeleaState createState() => ImagenPeleaState();
}

// Clase que representa el estado del widget ImagenTop.
class ImagenPeleaState extends State<ImagenPelea>
    ...
    return GestureDetector(
      ...
      onTapUp: (_) {
        ...
        Navigator.push(
          context,
          MaterialPageRoute(
            builder: (context) => InfoPelea(imageData: widget.imageData),
          ),
        );
      },
      onTapCancel: () {
...
class InfoPelea extends StatelessWidget {
  final Map<dynamic, dynamic> imageData;

  const InfoPelea({Key? key, required this.imageData}) : super(key: key);
